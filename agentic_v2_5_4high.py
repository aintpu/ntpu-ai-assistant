...
def _data_path(name: str) -> str:
    ...
    兩處都找不到時回傳根目錄路徑，讓後續 os.path.exists 判斷維持原本的略過行為。"""
    for cand in (os.path.join(BASE_DIR, name),
                 os.path.join(BASE_DIR, "crawler_data", name)):
        if os.path.exists(cand):
            return cand
    return os.path.join(BASE_DIR, name)

FILE_PATH_ZH = _data_path("all_content_v2.md")
FILE_PATH_EN = _data_path("all_content_en_v2.md")
REGULATIONS_PATH = _data_path("ALL_files_2.md")
FILE_INDEX_PATH = os.path.join(BASE_DIR, "file_index.json")
# 讀寫都用絕對路徑，避免 cwd 不同導致寫入與載入指到不同檔案
...
CORRECTION_DEPT_RE = re.compile(r"^-\s*\*\*所屬處室\*\*：\s*([A-Za-z]+)", re.M)

def _parse_correction_dept(chunk: str) -> str:
    m = CORRECTION_DEPT_RE.search(chunk)
    code = (m.group(1).lower() if m else "")
    return code if code in DEPT_NAMES else "ope"

def _norm_reg_title(t: str) -> str:
    t = re.sub(r"\.(pdf|docx?|odt|ods)$", "", (t or "").strip(), flags=re.I)
    return re.sub(r"\s+", "", t)

def _meta_from_tags(url: str, tags: str, date: str) -> dict:
    return {
        "url": (url or "").strip(),
...
def _load_regulation_meta(dept: str) -> Dict[str, dict]:
    """讀 xlsx，回傳 {正規化標題: {url, category, type, date}}；讀不到時回空 dict
    （僅缺 metadata，不影響全文入庫）
    ...
    try:
        ...
        for row in rows:
            ...
            out[_norm_reg_title(title)] = _meta_from_tags(
                url, str(d.get("tags") or ""), str(d.get("updated_date") or ""))
        wb.close()
        return out
...
def _load_admin_regulation_meta(dept: str) -> Dict[str, dict]:
    """行政單位彙整表：單一工作表 + 中文欄名，用「處室」欄篩出本處室的列"""
    dept_label = DEPT_ADMIN_XLSX_MAP.get(dept)
    if not dept_label or not os.path.exists(ADMIN_REGULATION_XLSX):
        return {}
    try:
        import openpyxl
        wb = openpyxl.load_workbook(ADMIN_REGULATION_XLSX, data_only=True, read_only=True)
        rows = wb[wb.sheetnames[0]].iter_rows(values_only=True)
        hdr = [str(c) if c else "" for c in next(rows)]
        out = {}
        for row in rows:
            d = dict(zip(hdr, row))
            if str(d.get("處室") or "").strip() != dept_label:
                continue
            title = str(d.get("法規名稱") or "").strip()
            if not title:
                continue
            # 欄名以「檔案連結」為準，另相容可能出現的「網址」欄
            url = str(d.get("檔案連結") or d.get("網址") or "").strip()
            out[_norm_reg_title(title)] = _meta_from_tags(
                url, str(d.get("標籤") or ""), str(d.get("上傳日期") or ""))
        wb.close()
        return out
    except Exception as e:
        print(f"[系統] 讀取行政單位法規彙整 xlsx 失敗（{e}），{dept} 法規將無 URL metadata")
        return {}
...
# ==========================================
# 索引快取：資料檔指紋相同時直接載入現成索引，
# 省全量 embedding（約 50 萬 tokens）與數分鐘啟動時間。
#
# 指紋用「檔案內容雜湊」而非 mtime：mtime 撐不過 git checkout（git 不保存 mtime）
...
def _data_fingerprint() -> str:
    import hashlib
    paths = [FILE_PATH_ZH, REGULATIONS_PATH, CORRECTIONS_PATH,
             REGULATION_XLSX, ADMIN_REGULATION_XLSX] + \
            [p for _, p in CRAWLER_SOURCES] + [p for _, p in REGULATION_SOURCES]
    parts = []
    for p in paths:
        if os.path.exists(p):
            h = hashlib.md5()
...
class OPEIndex:
    ...
    def build(self):
        ...
        # ✨ 3. 讀取使用者回饋修正檔 (corrections.md)
        #    必須放在上面的 dept="ope" 覆寫「之後」：修正紀錄是跨處室的，
        #    被統一標成 ope 的話，其他處室的問題會因 dept 過濾而永遠檢索不到自己的修正紀錄。
        ...
        if os.path.exists(CORRECTIONS_PATH):
            with open(CORRECTIONS_PATH, "r", encoding="utf-8-sig") as f:
                ...
                for chunk in correction_chunks:
                    if "## 修正紀錄" in chunk:
                        c_dept = _parse_correction_dept(chunk)
                        n_corr[c_dept] += 1
                        docs.append(Document(
                            page_content=chunk.strip(),
                            metadata={"page": "系統修正紀錄", "type": "correction",
                                      "title": "使用者糾正資訊", "dept": c_dept}
                        ))
        if n_corr:
        ...
        for dept, path in REGULATION_SOURCES:
            if not os.path.exists(path):
                print(f"[系統] 找不到 {dept} 法規檔：{path}，略過")
                continue
            with open(path, "r", encoding="utf-8-sig") as f:
                reg_docs = parse_dept_regulations(f.read(), dept)
            docs.extend(reg_docs)
            print(f"[系統] {DEPT_NAMES.get(dept, dept)} 接入法規切塊 {len(reg_docs)} 筆")
...
def _is_dept_related(query: str, correction: str, dept: str = None) -> bool:
    """
    用 LLM 判斷修正內容是否屬於本輪服務處室的業務範疇。
    只做二元分類，不驗證內容真偽。
    """
    dept_name = DEPT_NAMES.get(dept or "", "")
    scope = f"「{dept_name}」的業務" if dept_name else "國立臺北大學任一服務處室的業務"
    try:
        out = llm_adapter.complete(
            [{"role": "system", "content": (
                f"你是一個分類器。簡單判斷以下內容是否與{scope}相關，或是臺北大學相關，"
                "包含：場地借用、課程、活動、師資、學籍、選課、成績、獎助學金、住宿、"
                "法規、公告、表單、交通、位置等。"
...
def tool_record_correction(original_query: str, correction_info: str, dept: str = None) -> str:
    ...
    修正紀錄會標記所屬處室，建索引時還原成該筆文件的 dept metadata，
    確保下次同處室的問題檢索得到（dept 過濾會刷掉其他處室的文件）。
    """
    dept = dept if dept in DEPT_NAMES else None
    dept_name = DEPT_NAMES.get(dept or "", "")

    # 驗證是否屬於本輪服務處室的業務範疇
    if not _is_dept_related(original_query, correction_info, dept):
        scope = f"與{dept_name}無關" if dept_name else "與本系統服務的處室無關"
        return (
            f"這筆修正內容看起來{scope}，因此不予寫入。"
            "若確認屬於本校相關資訊，請提供更具體的說明。"
        )

    # 通過驗證，正常寫入
    file_path = os.path.join(BASE_DIR, "corrections.md")
    ...
    with open(file_path, "a", encoding="utf-8") as f:
        ...
        # 舊紀錄沒有這一行，解析端會 fallback 成 ope（原本的行為）
        f.write(f"- **所屬處室**：{dept or 'ope'}（{dept_name or DEPT_NAMES['ope']}）\n")
